#!/usr/bin/env python3
"""
Codex Harness — MetaGPT Agent 适配器

MetaGPT Agent 的适配器实现。
...
class MetaGPTMemory(MemoryInterface):
    def __init__(self):
...
class MetaGPTContext(ContextInterface):
    def __init__(self):
...
class MetaGPTTools(ToolInterface):
    def __init__(self):
...
class MetaGPTConfig(ConfigInterface):
    def __init__(self):
...
class MetaGPTAdapter(AgentAdapter):
    def __init__(self):
        self.memory = MetaGPTMemory()
        self.context = MetaGPTContext()
        self.tools = MetaGPTTools()
        self.config = MetaGPTConfig()
    ...
    def get_capabilities(self) -> List[str]:
        return ["multi_agent", "role_playing", "planning", "code_generation", "tool_calling"]

#!/usr/bin/env python3
"""
Codex Harness — BabyAGI Agent 适配器

BabyAGI Agent 的适配器实现。
...
class BabyAGIMemory(MemoryInterface):
    def __init__(self):
...
class BabyAGIContext(ContextInterface):
    def __init__(self):
...
class BabyAGITools(ToolInterface):
    def __init__(self):
...
class BabyAGIConfig(ConfigInterface):
    def __init__(self):
...
class BabyAGIAdapter(AgentAdapter):
    def __init__(self):
        self.memory = BabyAGIMemory()
        self.context = BabyAGIContext()
        self.tools = BabyAGITools()
        self.config = BabyAGIConfig()
    ...
    def get_capabilities(self) -> List[str]:
        return ["task_creation", "task_prioritization", "task_execution", "memory", "reasoning"]

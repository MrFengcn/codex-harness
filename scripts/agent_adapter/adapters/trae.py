#!/usr/bin/env python3
"""
Codex Harness — Trae 适配器

字节跳动 Trae AI IDE 的适配器实现。
...
class TraeMemory(MemoryInterface):
    def __init__(self):
...
class TraeContext(ContextInterface):
    def __init__(self):
...
class TraeConfig(ConfigInterface):
    def __init__(self):
...
class TraeAdapter(AgentAdapter):
    """字节跳动 Trae AI IDE 适配器"""
    def __init__(self):
        self.memory = TraeMemory()
        self.context = TraeContext()
        self.tools = TraeTools()
    ...
    def get_capabilities(self) -> List[str]:
        return [
            ...
            "vscode_extension", "ai_chat", "inline_edit", "byte_dance_ecosystem",
        ]
